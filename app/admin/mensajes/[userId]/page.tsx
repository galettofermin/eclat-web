...
export default function AdminConversacion() {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-80px)] -m-6 md:-m-8">
      {/* Header */}
      <div className="flex items-center gap-3 px-6 py-4 border-b border-black/[0.06] bg-white shrink-0">
        <Link href="/admin/mensajes" className="text-[#6E6E73] hover:text-[#0A0A0A] transition-colors">
        ...
        {otro && (
          <>
            <div className="w-9 h-9 rounded-full bg-[#DCEFE8] flex items-center justify-center text-[#2F7D6B] font-semibold text-[14px] shrink-0">
              {otro.nombre.charAt(0).toUpperCase()}
            </div>
            <div>
              <p className="text-[14px] font-semibold text-[#0A0A0A]">{otro.nombre}</p>
      ...
      <div className="flex-1 overflow-y-auto px-6 py-4 bg-[#F9F9F9]">
        {loading ? (
          ...
          <div className="text-center py-12">
            <p className="text-[14px] text-[#6E6E73]">Iniciá la conversación con este alumno.</p>
          </div>
        ) : (
          mensajes.map(m => {
      ...
      {/* Input */}
      <div className="px-6 py-3 border-t border-black/[0.06] bg-white shrink-0">
        <form onSubmit={enviar} className="flex gap-2 items-end">
          <textarea
            ...
            placeholder="Escribí un mensaje al alumno..."
            rows={1}
            className="flex-1 resize-none px-4 py-2.5 rounded-xl border border-black/10 text-[14px] focus:outline-none focus:border-[#2F7D6B] transition-colors max-h-32"
          />
          <button
            type="submit"
